...
def main(args):
    if None == args.queryvcf == args.querybed and not args.passonly and not args.failonly:
    ...
    for rec in vcfin:
        ...
        if args.passonly and rec.FILTER is not None:
            output = False

        if args.failonly and rec.FILTER is None:
            output = False
