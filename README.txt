...
Comparison can be parallelized (currently SMP):

usage: parallel_cmp.py [-h] [-m MASKFILE] [-o OUTDIR] [-t TRUTH] -f FAI
                       [-p PROCS] [-v] [--skip_merge]
                       <vcf_file> <vcf_file>
...
optional arguments:
  ...
  -t TRUTH, --truth TRUTH
                        also compare results to a "truth" VCF (should be
                        sorted and tabix-indexed)
  -f FAI, --fai FAI     .fai file generated by samtools faidx
  -p PROCS, --procs PROCS
                        number of jobs
  -v, --verbose         verbose mode for debugging
  --skip_merge          skip VCF merge step
